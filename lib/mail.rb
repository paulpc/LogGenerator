...
module Sources
  class Mail < Syslog
    attr_reader :email_directory
   def initialize(host="mail_"+rand(999999).to_s.rjust(6,"0"))
    @host=host      
            if $firewall
         @ip= $firewall.assign("mail_servers")
        else
...
# log successful and unsuccessful emails receipts
   def log_received(sender={:host=>rand_domain("mail"),:ip=>rand_ip()},recipient=@email_directory.values.sample,accept=false)
       pid=(rand(2000)+1000).to_s
      msgid="<#{(("a".."z").to_a+("0".."9").to_a).sample(12).join}-#{27000000+rand(99999)}-#{(("a".."z").to_a+("0".."9").to_a).sample(16).join}>"
      $firewall.traffic(sender[:ip],@ip,"smtp") if $firewall
      if accept
        ...
        syslog_log(get_time(),"sendmail[#{pid}]", "p79DwHsd#{pid}: Milter delete: rcpt <#{recipient}>")
    ...
    def log_sent(recipient={:ip=>rand_ip(),:host=>rand_domain("mail")}, sender=@email_directory.values.sample, accept=false)
       pid=(rand(2000)+1000).to_s
       msgid="<#{(("a".."z").to_a+("0".."9").to_a).sample(12).join}-#{27000000+rand(99999)}-#{(("a".."z").to_a+("0".."9").to_a).sample(16).join}>"
       $firewall.traffic(@ip,recipient[:ip],"smtp") if $firewall
      if accept
    ...
    #generates email white noise. Will generate an email every once in a while to an apropriate domain outside ours and some mails within the agency
    def white_noise()
      while $all_normal
        source=@email_directory.keys.sample
        name=Attacks::userlist().sample
        foreign_domain=rand_domain("mail")
        foreign_email="#{name.downcase}@#{foreign_domain.sub(/mail\./,"")}"
        if Time.now.sec%2  == 0 
        log_sent({:host=>foreign_domain,:ip=>rand_ip(),:mail=>foreign_email},@email_directory[source],true)
        else
        log_received({:host=>foreign_domain,:ip=>rand_ip(),:mail=>foreign_email},@email_directory[source],true)
        end        
        sleep(rand(10))
      end
    end
